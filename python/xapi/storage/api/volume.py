...
class Volume_server_dispatcher:
    ...
    def snapshot(self, args):
        ...
        if type(results['uri']) <> type([]):
            raise (TypeError("string list", repr(results['uri'])))
        for tmp_13 in results['uri']:
            if type(tmp_13) <> type("") and type(tmp_13) <> type(u""):
                raise (TypeError("string", repr(tmp_13)))
        if type(results['keys']) <> type({}):
            raise (TypeError("(string * string) list", repr(results['keys'])))
    ...
    def compare(self, args):
        ...
        if type(key) <> type("") and type(key) <> type(u""):
            raise (TypeError("string", repr(key)))
        if not(args.has_key('key2')):
            raise UnmarshalException('argument missing', 'key2', '')
        key2 = args["key2"]
        if type(key2) <> type("") and type(key2) <> type(u""):
            raise (TypeError("string", repr(key2)))
        results = self._impl.compare(dbg, sr, key, key2)
        if type(results['blocksize']) <> type(0):
            raise (TypeError("int", repr(results['blocksize'])))
        if type(results['ranges']) <> type([]):
            raise (TypeError("(int64 * int64) list", repr(results['ranges'])))
        for tmp_18 in results['ranges']:
            if (type(tmp_18) is tuple) and tmp_18.length==2:
                if not(is_long(tmp_18[0])):
                    raise (TypeError("int64", repr(tmp_18[0])))
                if not(is_long(tmp_18[1])):
                    raise (TypeError("int64", repr(tmp_18[1])))
        return results
    def _dispatch(self, method, params):
        ...
        elif method == "Volume.compare":
            return success(self.compare(args))
class Volume_skeleton:
    ...
    def compare(self, dbg, sr, key, key2):
        """Operations which operate on volumes (also known as Virtual Disk Images)"""
        raise Unimplemented("Volume.compare")
class Volume_test:
    """Operations which operate on volumes (also known as Virtual Disk Images)"""
    def __init__(self):
        pass
    ...
    def compare(self, dbg, sr, key, key2):
        """Operations which operate on volumes (also known as Virtual Disk Images)"""
        result = {}
        result["blocklist"] = { "blocksize": 0L, "ranges": [ [], [] ] }
        return result
...
class Volume_commandline():
    ...
    def _parse_compare(self):
        ...
        if ('--json' in sys.argv or '-j' in sys.argv):
            ...
            jsondict['json'] = True
            return jsondict
        parser = argparse.ArgumentParser(description='[compare sr volume1 volume2] compares the two volumes and returns a result of type blocklist that describes the differences between the two volumes. If the two volumes are unrelated, or the second volume does not exist, the result will be a list of the blocks that are non-empty in volume1. If this information is not available to the plugin, it should return a result indicating that all blocks are in use.')
        parser.add_argument('-j', '--json', action='store_const', const=True, default=False, help='Read json from stdin, print json to stdout', required=False)
        parser.add_argument('dbg', action='store', help='Debug context from the caller')
        parser.add_argument('sr', action='store', help='The Storage Repository')
        parser.add_argument('key', action='store', help='The volume key')
        parser.add_argument('key2', action='store', help='The volume key')
        return vars(parser.parse_args())
    def create(self):
        use_json = False
        try:
    ...
    def compare(self):
        use_json = False
        try:
            request = self._parse_compare()
            use_json = 'json' in request and request['json']
            results = self.dispatcher.compare(request)
            print json.dumps(results)
        except Exception, e:
            if use_json:
                xapi.handle_exception(e)
...
class SR_server_dispatcher:
    ...
    def probe(self, args):
        ...
        if type(results['srs']) <> type([]):
            raise (TypeError("{ sr: string; name: string; description: string; free_space: int64; total_space: int64; datasources: string list; clustered: bool; health: | Healthy (string) (** Storage is fully available *) | Recovering (string) (** Storage is busy recovering, e.g. rebuilding mirrors *); } list", repr(results['srs'])))
        for tmp_20 in results['srs']:
            if type(tmp_20['sr']) <> type("") and type(tmp_20['sr']) <> type(u""):
                raise (TypeError("string", repr(tmp_20['sr'])))
            if type(tmp_20['name']) <> type("") and type(tmp_20['name']) <> type(u""):
                raise (TypeError("string", repr(tmp_20['name'])))
            if type(tmp_20['description']) <> type("") and type(tmp_20['description']) <> type(u""):
                raise (TypeError("string", repr(tmp_20['description'])))
            if not(is_long(tmp_20['free_space'])):
                raise (TypeError("int64", repr(tmp_20['free_space'])))
            if not(is_long(tmp_20['total_space'])):
                raise (TypeError("int64", repr(tmp_20['total_space'])))
            if type(tmp_20['datasources']) <> type([]):
                raise (TypeError("string list", repr(tmp_20['datasources'])))
            for tmp_21 in tmp_20['datasources']:
                if type(tmp_21) <> type("") and type(tmp_21) <> type(u""):
                    raise (TypeError("string", repr(tmp_21)))
            if type(tmp_20['clustered']) <> type(True):
                raise (TypeError("bool", repr(tmp_20['clustered'])))
            if tmp_20['health'][0] not in ["Healthy","Recovering"]:
                raise (TypeError("| Healthy (string) (** Storage is fully available *) | Recovering (string) (** Storage is busy recovering, e.g. rebuilding mirrors *)", repr(tmp_20['health'])))
            if tmp_20['health'][0] == 'Healthy':
                if type(tmp_20['health'][1]) <> type("") and type(tmp_20['health'][1]) <> type(u""):
                    raise (TypeError("string", repr(tmp_20['health'][1])))
            elif tmp_20['health'][0] == 'Recovering':
                if type(tmp_20['health'][1]) <> type("") and type(tmp_20['health'][1]) <> type(u""):
                    raise (TypeError("string", repr(tmp_20['health'][1])))
        if type(results['uris']) <> type([]):
            raise (TypeError("string list", repr(results['uris'])))
        for tmp_19 in results['uris']:
    ...
    def create(self, args):
        ...
        for tmp_22 in configuration.keys():
            if type(tmp_22) <> type("") and type(tmp_22) <> type(u""):
                raise (TypeError("string", repr(tmp_22)))
        for tmp_22 in configuration.values():
            if type(tmp_22) <> type("") and type(tmp_22) <> type(u""):
                raise (TypeError("string", repr(tmp_22)))
    ...
    def stat(self, args):
        ...
        for tmp_23 in results['datasources']:
            if type(tmp_23) <> type("") and type(tmp_23) <> type(u""):
                raise (TypeError("string", repr(tmp_23)))
        if type(results['clustered']) <> type(True):
            raise (TypeError("bool", repr(results['clustered'])))
        if results['health'][0] not in ["Healthy","Recovering"]:
            raise (TypeError("| Healthy (string) (** Storage is fully available *) | Recovering (string) (** Storage is busy recovering, e.g. rebuilding mirrors *)", repr(results['health'])))
    ...
    def ls(self, args):
        ...
        if type(results) <> type([]):
            raise (TypeError("{ key: string; uuid: string option; name: string; description: string; read_write: bool; virtual_size: int64; physical_utilisation: int64; uri: string list; keys: (string * string) list; } list", repr(results)))
        for tmp_24 in results:
            if type(tmp_24['key']) <> type("") and type(tmp_24['key']) <> type(u""):
                raise (TypeError("string", repr(tmp_24['key'])))
            if tmp_24['uuid'] <> None:
                if type(tmp_24['uuid']) <> type("") and type(tmp_24['uuid']) <> type(u""):
                    raise (TypeError("string", repr(tmp_24['uuid'])))
            if type(tmp_24['name']) <> type("") and type(tmp_24['name']) <> type(u""):
                raise (TypeError("string", repr(tmp_24['name'])))
            if type(tmp_24['description']) <> type("") and type(tmp_24['description']) <> type(u""):
                raise (TypeError("string", repr(tmp_24['description'])))
            if type(tmp_24['read_write']) <> type(True):
                raise (TypeError("bool", repr(tmp_24['read_write'])))
            if not(is_long(tmp_24['virtual_size'])):
                raise (TypeError("int64", repr(tmp_24['virtual_size'])))
            if not(is_long(tmp_24['physical_utilisation'])):
                raise (TypeError("int64", repr(tmp_24['physical_utilisation'])))
            if type(tmp_24['uri']) <> type([]):
                raise (TypeError("string list", repr(tmp_24['uri'])))
            for tmp_26 in tmp_24['uri']:
                if type(tmp_26) <> type("") and type(tmp_26) <> type(u""):
                    raise (TypeError("string", repr(tmp_26)))
            if type(tmp_24['keys']) <> type({}):
                raise (TypeError("(string * string) list", repr(tmp_24['keys'])))
            for tmp_25 in tmp_24['keys'].keys():
                if type(tmp_25) <> type("") and type(tmp_25) <> type(u""):
                    raise (TypeError("string", repr(tmp_25)))
            for tmp_25 in tmp_24['keys'].values():
                if type(tmp_25) <> type("") and type(tmp_25) <> type(u""):
                    raise (TypeError("string", repr(tmp_25)))
        return results
    def _dispatch(self, method, params):
